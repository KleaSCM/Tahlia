...
PythonBridge::PythonBridge() 
    : config_({"tahlia_core", "1.0.0", "Elegant asset management for Blender", {}, {}, true, true, 10})
...
BlenderContext PythonBridge::captureContext() const {
    /**
     * @brief Captures the current Blender context state for preservation.
     *        Uses Python bridge to get real Blender context data.
     *
    ...
    try {
        ...
        handlePythonException(e);
        // Fallback to basic context if Python integration fails
        context.selected_objects = {"Cube", "Camera", "Light"};
        context.active_object = "Cube";
        context.view_layer = "View Layer";
        context.mode = "OBJECT";
        context.viewport_settings["shading"] = "MATERIAL";
        context.viewport_settings["overlay"] = "WIREFRAME";
        context.visible_collections = {"Collection"};
...
bool PythonBridge::restoreContext(const BlenderContext& context) {
    /**
     * @brief Restores a previously captured Blender context state.
     *        Uses Python bridge to restore real Blender context.
     *
...
PythonResult PythonBridge::importAssetPython(const std::string& asset_path, const std::map<std::string, std::string>& options) {
    /**
     * @brief Python-friendly wrapper for importing a single asset.
     *        Uses Python bridge to perform actual Blender import operations.
     *
...
PythonResult PythonBridge::createMaterialPython(const std::string& name, const std::map<std::string, std::string>& options) {
    /**
     * @brief Python-friendly wrapper for creating materials.
     *        Uses Python bridge to perform actual Blender material creation.
     *
...
PythonResult PythonBridge::createPBRMaterialPython(const std::string& name, const std::map<std::string, std::string>& options) {
    /**
     * @brief Python-friendly wrapper for creating PBR materials.
     *        Uses Python bridge to perform actual Blender PBR material creation.
     *
     * @param name Name of the material to create.
     * @param options Map containing material options.
     * @return PythonResult containing the material creation result.
